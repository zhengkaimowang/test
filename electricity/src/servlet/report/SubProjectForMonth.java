...
public class SubProjectForMonth extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public SubProjectForMonth() {
		super();
	}
			...
			throws ServletException, IOException 
	{
		...
		try {
			list = DAOFactory.getProjectDAO().findSubProject(code);
		} catch (Exception e) {
		...
		for(int temp = 0;temp < length;temp ++)
		{
			...
			for(int tempDay = 0;tempDay < iDate;tempDay ++)
			{
				...
				try {
					equipment = DAOFactory.getEquipmentDAO().findBySubPro(name);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
		...
		request.getRequestDispatcher("subMonth.jsp").forward(request, response);
	}
