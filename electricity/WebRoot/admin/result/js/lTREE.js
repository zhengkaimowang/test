...
lDOM.BV=(function(){   //执行这个匿名函数,创建BV对象字面量
	var u="indexOf",
		n=navigator,  //提供用户所使用的浏览器以及操作系统等信息
		a=n.userAgent.toLowerCase(), //返回由客户机发送服务器的 user-agent 头部的值,并变成小写
...
lTREE.prototype={
	...
	_push4IE:function(s){
		...
		if(s.getElementsByTagName(DD).length<1){
			...
			//by michael.li add 2009-03-21
//			alert(b.length);
			if (b.length==1)
			{
			    //alert(b.length);
			    lDOM.addClass(b[0],lc);
			}
